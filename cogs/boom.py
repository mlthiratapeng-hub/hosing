...
@bot.tree.command(name="boom_webhook")
async def boom_webhook(interaction: discord.Interaction, webhook_url: str, count: int):
            ...
            async with aiohttp.ClientSession() as session:
                ...
                for i in range(count):
                    if not self.is_running:
                        break
                    try:
                        await webhook.send(f"สแปมข้อความ #{i+1}")
